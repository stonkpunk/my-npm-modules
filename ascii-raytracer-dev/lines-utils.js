...
function pt2XLines(pt,s, color= [Math.random(),Math.random(),Math.random()]){
    var sol = [];
    var l0 = [addPts(pt,[-s,0,-s]),addPts(pt,[s,0,s])];
    var l1 = [addPts(pt,[s,0,-s]),addPts(pt,[-s,0,s])];
    if(color || pt.color){
        pt.color = pt.color || color;
        l0.color = pt.color || color ;
        l1.color = pt.color || color ;
    }
    sol.push(l0,l1);
    return sol;
}

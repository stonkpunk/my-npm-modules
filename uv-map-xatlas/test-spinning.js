...
var xatlasTool = require('./index.js');

xatlasTool.atlasForIndexedTriangles(bunny, function(res) {

    var res = xatlasTool.atlasToTrianglesObjsList(res[0]);

    var previewTriangles = res.trianglesUVPreview;

    //TODO try to do "packing" operation? result looks good but has separate "islands"
    //try box packing the islands if their bounds do not overlap...
...
function generateRndColors(){
    triangleColorsFlat = triangles.map(rt.randomColor); //1 color per triangle [r,g,b] -- [255, 255, 255] for white etc
    triangleColorsVerts = triangles.map(t=>[rt.randomColor(),rt.randomColor(),rt.randomColor()]); //1 color per vert, each triangle gets [[r,g,b],[r,g,b],[r,g,b]]
}

var triangleColorsFlat = [];
var triangleColorsVerts = [];
generateRndColors();
...
function renderOne(){
    ...
    var config = {
        ...
        edgesOnly:true, //default false, wireframe more

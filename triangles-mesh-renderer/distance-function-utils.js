...
function trianglesWithinAabbFunc(tris, radius=0.01){
    var trianglesRTree = triangles2RTree(tris);//console.log("TRI RTREE?", trisRTree);
    var s = radius;
    return function(aabb){
        var potentialTris = trianglesRTree.search(sector2RTreeObj(aabb,s)).map(s=>s.triangle);//(x,y,z,ndir[0],ndir[1],ndir[2],1000);//.map(o=>o.triangle);
